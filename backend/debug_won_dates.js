...
async function checkWonDates() {
    console.log(`\n\n=== Checking Dates of Won Phase (${WON_PHASE_ID}) ===`);

    // Default fetch of phase cards (ordered by position usually, which implies recent movement)
    const query = `
        {
    ...
    try {
        ...
        cards.forEach((c, i) => {
            console.log(`[${i + 1}] ${c.title} | Finished: ${c.finished_at} | Updated: ${c.updated_at} | Created: ${c.createdAt}`);
        });
